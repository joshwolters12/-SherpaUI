...
app.on('ready', () => {
    ...
    // childProcess.exec('node starterReactVR/node_modules/react-native/local-cli/cli.js start', function(error, stdout, stderr) {
    //     console.log(stdout);
    //     console.log(stderr);
    //     console.log(error);
    //     console.log('HELLOOO!!!');
    ...
    childProcess.exec('launchctl setenv PATH "$PATH" && electron server.js', {env: productionEnv})
    // childProcess.execFileSync(process.env.SHELL, ['-i', '-c', 'launchctl setenv PATH "$PATH"'])
    // var gulpPath = path.join(__dirname, '../', 'server.js');
    // childProcess.spawn(gulpPath, {cwd: 'app'});
