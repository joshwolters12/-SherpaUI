...
app.on('ready', () => {
    ...
    submenu: [{
      ...
      click: (item, focusedWindow) => {
        dialog.showOpenDialog(mainWindow, {
          ...
          fs.readFile('./starterReactVR/myjsonfile.json', function(err, data) {
            ...
            stateObj.right["text"] = ''
            fs.writeFile('./myjsonfile.json', JSON.stringify(stateObj, null, 2), function(err) {
              if (err) return console.log(err)
            })
          })
          fs.copy(filePaths[0], './static_assets/' + newImage)
          mainWindow.reload()
        })
      }
    }, {
      ...
      click: () => {
        dialog.showOpenDialog({
          ...
          defaultPath: "./saved_projects/"
        }, function(filePaths) {
          if (filePaths) {
            let fileToOpen = filePaths[0].split('/').pop()
            fs.readFile('./saved_projects/' + fileToOpen, function(err, data) {
              if (err) return console.log(err)
        ...
        click: (item, focusedWindow) => {
          dialog.showSaveDialog({
            ...
            if (filename) {
              ...
              fs.copy('./myjsonfile.json', './saved_projects/' + fileToSave, function(err) {
                if (err) return console.log(err)
              })
            }
          })
  ...
  exec('node node_modules/react-native/local-cli/cli.js start')
  ...
  setTimeout(function() {
    mainWindow.loadURL('file://' + __dirname + '/sherpa.html');
  }, 500)
